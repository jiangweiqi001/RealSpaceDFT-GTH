"""Focused H2 follow-up v2 for adaptive Hartree exterior/boundary comparison.

This is not a final benchmark.
The current goal is to judge whether H2 at fixed bond length is still box-
dominated, whether the new ``uniform_exterior`` boundary mode reduces box
sensitivity relative to ``multipole_dirichlet``, and whether the current next
priority should remain boundary/exterior treatment or shift to the adaptive
kinetic/Laplacian discretization.
"""
...
def build_results():
    ...
    mode_specs = [
        ...
        {"name": "uniform_exterior", "backend": AdaptiveBackend(hartree_boundary_mode="uniform_exterior")},
    ]
...
def build_boundary_summary(results):
    ...
    for box in BOXES:
        ...
        uniform_ext = index[('uniform_exterior', box)]
        ...
        summary.append({
            ...
            'uniform_ext_minus_zero': delta(uniform_ext, zero),
            'uniform_ext_minus_multi': delta(uniform_ext, multi),
            'uniform_ext_minus_mono_charge': delta(uniform_ext, mono_charge),
        })
    return summary
...
def print_boundary_summary(summary):
    ...
    header = (
        f"{'Box':>5} {'MonoCharge-Zero':>18} {'Multi-Zero':>14} {'UniformExt-Zero':>18} "
        f"{'UniformExt-Multi':>18} {'UniformExt-MonoCharge':>24}"
    )
    print(header)
    print("-" * len(header))
    for row in summary:
        print(
            f"{fmt_float(row['box'], 5, 1)} {fmt_sci(row['mono_charge_minus_zero'], 18, 3)} "
            f"{fmt_sci(row['multi_minus_zero'], 14, 3)} {fmt_sci(row['uniform_ext_minus_zero'], 18, 3)} "
            f"{fmt_sci(row['uniform_ext_minus_multi'], 18, 3)} {fmt_sci(row['uniform_ext_minus_mono_charge'], 24, 3)}"
        )
...
def diagnose(results, drift_summary, boundary_summary):
    ...
    max_uniform_ext_vs_multi = max(abs(row['uniform_ext_minus_multi']) for row in boundary_summary if row['uniform_ext_minus_multi'] is not None)
    max_uniform_ext_vs_mono_charge = max(abs(row['uniform_ext_minus_mono_charge']) for row in boundary_summary if row['uniform_ext_minus_mono_charge'] is not None)
    max_boundary_vs_zero = max(abs(row['uniform_ext_minus_zero']) for row in boundary_summary if row['uniform_ext_minus_zero'] is not None)

    zero_drift = drift_by_mode.get('zero_dirichlet', float('inf'))
    mono_drift = drift_by_mode.get('monopole_charge_center', float('inf'))
    multi_drift = drift_by_mode.get('multipole_dirichlet', float('inf'))
    uniform_ext_drift = drift_by_mode.get('uniform_exterior', float('inf'))
    best_boundary_box_drift = min(mono_drift, multi_drift, uniform_ext_drift)

    if uniform_ext_drift < 0.5 * min(mono_drift, multi_drift) and uniform_ext_drift <= 0.10:
        label = 'boundary-dominated'
    elif best_boundary_box_drift >= 0.20:
        label = 'box-dominated'
    elif uniform_ext_drift <= 0.05 and max_uniform_ext_vs_multi <= 0.05 and max_uniform_ext_vs_mono_charge <= 0.05:
    ...
    return {
        'label': label,
        'max_box_drift_zero': zero_drift,
        'max_box_drift_mono_charge': mono_drift,
        'max_box_drift_multi': multi_drift,
        'max_box_drift_uniform_ext': uniform_ext_drift,
        'max_uniform_ext_vs_multi': max_uniform_ext_vs_multi,
        'max_uniform_ext_vs_mono_charge': max_uniform_ext_vs_mono_charge,
...
def print_diagnosis(diag):
    ...
    print(f"max_box_drift_uniform_ext   = {diag['max_box_drift_uniform_ext']:.6f}")
    print(f"max_uniform_ext_vs_multi    = {diag['max_uniform_ext_vs_multi']:.6f}")
    print(f"max_uniform_ext_vs_mono     = {diag['max_uniform_ext_vs_mono_charge']:.6f}")
    print(f"max_boundary_vs_zero        = {diag['max_boundary_vs_zero']:.6f}")
    print()
    print("Interpret conservatively:")
    print("- If uniform_exterior substantially lowers box drift, keep improving boundary/exterior behavior.")
    print("- If uniform_exterior and multipole stay close while box drift remains large, the next suspect is likely finite-box wavefunction/exterior truncation rather than center choice.")
    print("- If all three nonzero boundary modes become close and box drift becomes small, the next suspect is likely the adaptive kinetic/Laplacian discretization.")


def main() -> int:
    print("=== H2 Boundary/Exterior Follow-up v2 ===")
    print("Note: this is not a final benchmark.")
    print("Note: the current goal is to judge at what box size H2 stops being box-dominated.")
    print("Note: the current goal is also to test whether uniform_exterior is more stable than multipole_dirichlet.")
    print("Setup: H2 fixed at d=1.4 Bohr.")
    print(f"Boxes: {BOXES}")
    ...
    all_ok &= check(
        ...
        'all four H2 boundary/exterior paths completed with finite outputs',
    )
    all_ok &= check(
        'electron_counts_ok',
